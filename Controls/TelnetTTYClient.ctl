...
'COPYRIGHT
'
'Copyright © 2005, 2007 Robert D. Riemersma, Jr.
'All Rights Reserved.
'
...
'               remote-echo on (Yes=True) or off.
'
...
'               errors that occur as well.
'
'
'ENUMS
'
'  TTC_ERRORS   TelnetTTYControl-specific error numbers.  See
'               below for details.
    ...
    Select Case strCommand
        Case telWILL_ECHO
            Select Case ClientInitiatedCommandState
                ...
                Case Else
                    RaiseEvent Error(TTC_PROTOCOL_ERROR, _
                                     "Unexpected WILL ECHO")
            End Select
            ClientInitiatedCommandState = cicIdle
        Case telWONT_ECHO
            Select Case ClientInitiatedCommandState
...
Public Sub Connect(Optional ByVal RemoteHost As String, _
    ...
    With wscTelnet
        ...
        If setsockopt(.SocketHandle, SOL_SOCKET, SO_OOBINLINE, 1, 4) = SOCKET_ERROR Then
            RaiseEvent Error(err.LastDllError, "Error setting SO_OOBINLINE socket option")
            Exit Sub
        Else
            .Close
            .LocalPort = 0
