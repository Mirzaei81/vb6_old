...
Begin VB.Form frmRecieved 
   ClientHeight    =   7065
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   12810
   Icon            =   "frmRecieved.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin VSFlex7LCtl.VSFlexGrid vsPayment 
      ...
      FormatString    =   $"frmRecieved.frx":A4C2
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   Begin RESIZEKITLibCtl.ResizeKit ResizeKit1 
      Height          =   480
      Left            =   3360
      OleObjectBlob   =   "frmRecieved.frx":A5FF
      TabIndex        =   11
      Top             =   120
      Width           =   480
   End
...
Private Sub Form_Load()
    ...
    If intVersion = Min Or intVersion = Normal Then
        ShowDisMessage "œ—Ì«›  »Â ’‰œÊﬁ œ— ‰”ŒÂ Â«Ì ÅÌ‘—› Â Ê »«·« — ÊÃÊœ œ«—œ", 1500
        Unload Me
        Exit Sub
    End If
...
'        .ColHidden(1) = True
        .ColHidden(2) = True
        .ColHidden(8) = True
        .ColHidden(3) = True  ' Uid _ Current User
...
'        .ColComboList(3) = s
        ...
        If clsArya.ExternalAccounting = False Then
            Set Rst = RunStoredProcedure2RecordSet("Get_RecievedType")
        Else
            Set Rst = RunStoredProcedure2RecordSet("Get_RecievedType_Acc")
        End If
        s = .BuildComboList(Rst, "Description", "Code")
        .ColComboList(4) = s
        
         For i = 0 To .Cols - 1
            .ColWidth(i) = Val(GetSetting(strMainKey, "vsRecieve", "Col" & i))
...
Private Sub vsPayment_AfterEdit(ByVal Row As Long, ByVal Col As Long)
    With vsPayment
        If (.Col = 4 And .TextMatrix(.Row, 4) = "") Then Exit Sub
        If .Row > 0 Then
            Select Case .Col
...
''''                       Set Rst = RunStoredProcedure2RecordSet("Get_Per")
''''                       S = .BuildComboList(Rst, "PersonName", "Ppno")
''''                       .ColComboList(6) = S
''''                       .ColComboList(8) = S
                    Else
                       .TextMatrix(.Row, 6) = ""
...
Public Sub Number()
    ...
    FWLed1.Tag = RunParametricStoredProcedure("Get_New_tblAcc_Recieved", Parameter)
    FWLed1.Value = FWLed1.Tag Mod 1000
    Exit Sub
ErrHandler:
    LogSave Me.Name, err, "Number"
...
With vsPayment
        For i = 1 To .Rows - 1
            If Not Trim(txtDate.ClipText) <> "" And Trim(.TextMatrix(i, 4)) <> "" And Trim(.TextMatrix(i, 5)) <> "" And Trim(.TextMatrix(i, 6)) <> "" And Trim(.TextMatrix(i, 7)) <> "" Then  'Trim(.TextMatrix(i, 3)) <> "" And
    ...
    Select Case MyFormAddEditMode
        Case AddMode
            ...
            For i = 1 To .Rows - 1
                If Trim(.TextMatrix(i, 4)) <> "" And Trim(.TextMatrix(i, 5)) <> "" And Trim(.TextMatrix(i, 7)) <> "" Then 'Trim(.TextMatrix(i, 3)) <> "" And
                    Parameter(0) = GenerateInputParameter("@No", adBigInt, 8, 0)
                    ...
                    Parameter(3) = GenerateInputParameter("@Uid", adInteger, 4, cmbPerson.ItemData(cmbPerson.ListIndex)) ' .TextMatrix(i, 3)
                    ...
                    Parameter(6) = GenerateInputParameter("@RecieveType", adInteger, 4, .TextMatrix(i, 4))
                    Parameter(7) = GenerateInputParameter("@Code_Bes", adBigInt, 8, IIf(.TextMatrix(i, 8) = "", 0, .TextMatrix(i, 8)))
        ...
        Case EditMode
            ...
            Result = RunParametricStoredProcedure("Update_tblAcc_Recieved", Parameter)
...
Private Sub vsPayment_AfterUserResize(ByVal Row As Long, ByVal Col As Long)
    For i = 0 To vsPayment.Cols - 1
        SaveSetting strMainKey, "vsRecieve", "Col" & i, vsPayment.ColWidth(i)
    Next
    ...
    With vsPayment
        If (MyFormAddEditMode = EnumAddEditMode.EditMode Or MyFormAddEditMode = EnumAddEditMode.AddMode) Then
             ...
             If .Col <> 6 Then
                .Select .Row, .Col
                .EditCell
             ElseIf .Col = 6 And (.TextMatrix(.Row, 4) = 0 Or .TextMatrix(.Row, 4) = 1 Or .TextMatrix(.Row, 4) = 2) Then
             ...
             ElseIf .Col = 6 And .TextMatrix(.Row, 4) = 3 Then
                ...
                If mvarcode <> 0 Then
                  ...
                  .TextMatrix(.Row, 8) = mvarcode
                   mvarcode = 0
                End If
             ElseIf .Col = 6 And .TextMatrix(.Row, 4) = 4 Then
...
Public Sub FirstKey()
    ...
    Set Rst = RunParametricStoredProcedure2Rec("Get_tblAcc_Recieved", Parameter)
    If Not (Rst.EOF = True And Rst.BOF = True) Then
    ...
    If Rst.State = adStateOpen Then
        If Rst.EOF = False Then
            ...
            '    vsPayment.TextMatrix(ii, 1) = Rst!Date '
                ...
                vsPayment.TextMatrix(ii, 3) = Rst!Uid 'nvcFirstName & " " & tempRst!nvcSurName
                vsPayment.TextMatrix(ii, 4) = Rst!RecieveType
                ...
                vsPayment.TextMatrix(ii, 8) = Rst!Code_Bes
...
Private Sub vsPayment_KeyPressEdit(ByVal Row As Long, ByVal Col As Long, KeyAscii As Integer)
        If KeyAscii = 13 And (Col = 3 Or Col = 4) Then
            Sendkey "{F4}", True
        End If
...
Public Sub Printing()
    ...
    CrystalReport1.ReportFileName = App.Path & "\Reports" & RepVer & "\RepRecieved_A4.rpt"
    ...
    If IsFileExist = False Then
            ...
            Exit Sub
    End If
    CrystalReport1.ReportTitle = "ê“«—‘  œ—Ì«› Â«Ì «‰Ã«„ ‘œÂ "
