...
Private Sub CmdSpec_Click()
    ...
    Unload frmAccess
    frmAccess.lblTitle.Caption = " رمز مدير را براي نمايش اطلاعات وارد كنيد"
    frmAccess.AccessStatus = LockShow
    frmAccess.Show vbModal
    
    If mVarAccessLevel <> 1 Then
        ShowDisMessage "فقط با دسترسي مدير مي توان وارد اين قسمت شد", 1500
    Else
...
Private Sub TinyEvent_TinyHIDDidconnect()
    ...
    MsgBox ("قفل از دستگاه جدا شد")
'    ShowDisMessage " قفل از دستگاه جدا شد . ", 2000
...
Private Sub SetTooltipText()
    ...
    With FWToolTip
        ...
        .Text(CmbVersion) = "با انتخاب ورژن هاي مختلف نرم افزار ميتوانيد از قابليت هاي آن مطلع شويد"
        .Text(Label7) = "با انتخاب ورژن هاي مختلف نرم افزار ميتوانيد از قابليت هاي آن مطلع شويد"
        .Text(LblLimited) = " نمايش مشخصات نرم افزار نصب شده "
        .Text(Frame2) = " نمايش مشخصات نرم افزار نصب شده "
        .Text(TxtUserName) = "نام کاربري را وارد کنيد."
        .Text(txtPassword) = "رمز عبور را وارد کنيد.  "
        .Text(cmd_FgaryaSite) = " اطلاع از آخرين محصولات و دستاوردهاي اف جي آريا در سايت "
...
Private Sub SetTooltipText_Demo()
    ...
    With FWToolTip
        ...
        If clsArya.LimitedVersion = True And HardLockFlagTrial = False Then
            .Text(CmbVersion) = "با انتخاب ورژن هاي مختلف نرم افزار ميتوانيد از قابليت هاي آن مطلع شويد"
            .Text(Label7) = "با انتخاب ورژن هاي مختلف نرم افزار ميتوانيد از قابليت هاي آن مطلع شويد"
            .Text(LblLimited) = "اين شرکت در صورت استفاده از سيستم آزمايشي براي نگهداري داده ها و پشتيباني سيستم هيچگونه مسئوليتي ندارد " & vbLf & "در صورت خريد نرم افزار داده ها در سيستم اصلي قابل استفاده مي باشند"
            .Text(Frame2) = "اين شرکت در صورت استفاده از سيستم آزمايشي براي نگهداري داده ها و پشتيباني سيستم هيچگونه مسئوليتي ندارد " & vbLf & "در صورت خريد نرم افزار داده ها در سيستم اصلي قابل استفاده مي باشند"
            .Text(TxtUserName) = "نام کاربري را وارد کنيد. مقدار پيش فرض  " & "0" & "  مي باشد"
            .Text(txtPassword) = "رمز عبور را وارد کنيد. مقدار پيش فرض  " & "100" & "  مي باشد"
        End If
        .Text(Frame1) = strPolicy
        .Text(cmd_FgaryaSite) = " اطلاع از آخرين محصولات و دستاوردهاي اف جي آريا در سايت "
    End With
...
'    mdifrm.sbStatusBar.Panels.Item(4).Text = "سال مالي : " + AccountYear
...
Private Sub Command1_Click(index As Integer)
    ...
    ElseIf index = 0 Then 'ok
            ...
            ' frmMsg.fwBtn(0).Visible = True
              ...
              If mvarMsgIdx = vbYes Then
                 ...
                 If Updated = True Then
                     ...
                     MsgBox "پاك كردن با موفقيت انجام شد" & vbLf & "برنامه را مجددا اجرا كنيد"
                     End
                 Else
                     MsgBox "اشكال در پاك سازي ديتا" & vbLf & "با شركت اف جي آريا تماس بگيريد"
                 End If
             Else
                 MsgBox "برنامه را مجددا اجرا كنيد"
                 End
             End If
        ElseIf Me.txtPassword.Text = strtemporary1 Then   'Zap Databse
            
              frmMsg.fwlblMsg.Caption = " آيا براي پاك كردن ديتا كالاها اطمينان داريد - ديتا فروش نيز پاك خواهد شد "
            ' frmMsg.fwBtn(0).Visible = True
              frmMsg.fwBtn(0).ButtonType = flwButtonOk
              frmMsg.fwBtn(1).ButtonType = flwButtonCancel
              frmMsg.fwBtn(0).Caption = "بلي"
              ...
              If mvarMsgIdx = vbYes Then
                 ...
                 If Updated = True Then
                     ...
                     If Updated = 1 Then
                        ...
                        End
                     Else
                        MsgBox "اشكال در پاك سازي ديتاي كالاها" & vbLf & "با شركت اف جي آريا  تماس بگيريد"
                        End
                     End If
                 Else
                     MsgBox "اشكال در پاك سازي ديتاي فروش" & vbLf & "با شركت اف جي آريا تماس بگيريد"
                     End
                 End If
             Else
                 MsgBox "برنامه را مجددا اجرا كنيد"
        ...
        ElseIf Me.txtPassword.Text = strtemporary2 Then   'Zap Databse
              ...
              frmMsg.fwlblMsg.Caption = "آيا براي پاك كردن كل ديتاها اطمينان داريد"
            ' frmMsg.fwBtn(0).Visible = True
              frmMsg.fwBtn(0).ButtonType = flwButtonOk
              frmMsg.fwBtn(1).ButtonType = flwButtonCancel
              frmMsg.fwBtn(0).Caption = "بلي"
         ...
         Else
            ...
            If Rst.EOF = True And Rst.BOF = True Then
                MsgBox "اشكال در خواندن شماره نسخه ديتا بيس " & vbLf & "با شركت اف جي آريا تماس بگيريد"
                End
            
            Else
                If Rst!ScriptNo < CurrentScriptNo Then
                    MsgBox "CurrentScriptNo = " & CurrentScriptNo & vbLf & "LastScriptNo = " & Rst!ScriptNo & vbLf & "اين نسخه از برنامه با شماره آخرين اسكريپت مربوطه هماهنگ نيست - " & vbLf & "دقت شود امكان خطا وجود دارد"
                End If
            End If
            ...
            If Rst.EOF = True And Rst.BOF = True Then
                ...
                frmMsg.fwlblMsg.Caption = "كلمه عبور درست نيست"
                frmMsg.fwBtn(0).Visible = False
                frmMsg.fwBtn(1).ButtonType = flwButtonOk
                frmMsg.fwBtn(1).Caption = "قبول"
                frmMsg.Show vbModal
                Me.txtPassword.Text = ""
                Me.txtPassword.SetFocus
                Exit Sub
             ElseIf Rst.Fields("ActDeAct") = 0 Then
                frmMsg.fwlblMsg.Caption = "کاربر غير فعال است "
                frmMsg.fwBtn(0).Visible = False
                frmMsg.fwBtn(1).ButtonType = flwButtonOk
                frmMsg.fwBtn(1).Caption = "قبول"
                frmMsg.Show vbModal
             ...
             ElseIf Rst.Fields("Job") = 9 Then
                frmMsg.fwlblMsg.Caption = "گارسون نمي تواند وارد سيستم شود"
                frmMsg.fwBtn(0).Visible = False
                frmMsg.fwBtn(1).ButtonType = flwButtonOk
                frmMsg.fwBtn(1).Caption = "قبول"
                frmMsg.Show vbModal
...
'                End If
                ...
                If clsStation.Language = Farsi Then
                    If strDelegate = "17" Then
                        mdifrm.StatusBar1.Panels(1).Text = " شعبه :" & CurrentBranchName & " | " & "به سفارش موسسه حسابرسي خاقاني -  03412521758" & " | " & "اجرا :  شرکت فن آورگسترآريا _ تلفن :" & "  +982188554455,+982188554466,+982188554477,+982188554488"
                    ElseIf strDelegate = "24" Then
                        mdifrm.StatusBar1.Panels(1).Text = "به سفارش شرکت تکين الکترونيک پاسارگاد - تلفن 22263035(09821) فکس  22916851" & " | "  ' & "اجرا :  شرکت فن آورگسترآريا _ تلفن :" & "  +982188554455,+982188554466,+982188554477,+982188554488"
                    ElseIf strDelegate = "56" Then
                        mdifrm.StatusBar1.Panels(1).Text = "In Order Of MoeinReklam Co " & " | " & " 07708615501 - 07708615502 - 07480151660 "    ' & "اجرا :  شرکت فن آورگسترآريا _ تلفن :" & "  +982188554455,+982188554466,+982188554477,+982188554488"
                    Else
                        mdifrm.StatusBar1.Panels(1).Text = " شعبه :" & CurrentBranchName & " | " & "اجرا: شرکت مهندسي فن آورگسترآريا _ تلفن :" & "  +982188554455,+982188554466,+982188554477,+982188554488"
                      '  mdifrm.StatusBar1.Panels(1).Text = "اجرا: شرکت مهندسي فن آورگسترآريا " '& "  +982188554455,+982188554466,+982188554477,+982188554488" ' " شعبه :" & CurrentBranchName & " | " &
                    End If
                ...
                Else
                 ...
                 If AccountYear <> left(clsDate.shamsi(Date), 4) Then
                     frmMsg.fwlblMsg.Caption = "آيا مايليد با سال مالي -" & AccountYear & " - که مخالف تاريخ فعلي است !ادامه دهيد؟ "
                     frmMsg.fwlblMsg.Caption = frmMsg.fwlblMsg.Caption & "در صورت انتخاب بلي سيستم با همين سال مالي ادامه مي دهد"
                     frmMsg.fwlblMsg.Caption = frmMsg.fwlblMsg.Caption & " و در صورت انتخاب خير سيستم سال مالي جديد را ساخته و با سال مالي جديد ادامه مي دهد"
                     frmMsg.fwBtn(0).ButtonType = flwButtonOk
                     frmMsg.fwBtn(0).Caption = "بلي"
                     ...
                     If mvarMsgIdx = vbNo Then
                         ...
                         If Result = 1 Then
                             frmMsg.fwlblMsg.Caption = "سال مالي جديد -" & AccountYear & "- ساخته شد."
                             frmMsg.fwBtn(0).Visible = False
                             frmMsg.fwBtn(1).ButtonType = flwButtonOk
                             frmMsg.fwBtn(1).Caption = "قبول"
                             frmMsg.Show vbModal
                         End If
                        frmMsg.fwlblMsg.Caption = "بايد كالاهاي انبار براي اين سال مالي اضافه شود."
                        frmMsg.fwBtn(0).Visible = False
                        frmMsg.fwBtn(1).ButtonType = flwButtonOk
                        frmMsg.fwBtn(1).Caption = "قبول"
                        frmMsg.Show vbModal
                        ...
                        frmMsg.fwlblMsg.Caption = " كالاهاي انبارمركزي براي اين سال مالي اضافه شد." & "براي ساير انبارها مدير سيستم اقدام نمايد."
                        frmMsg.fwBtn(0).Visible = False
                        frmMsg.fwBtn(1).ButtonType = flwButtonOk
                        frmMsg.fwBtn(1).Caption = "قبول"
                        frmMsg.Show vbModal
                ...
                If clsStation.SellerCaption = "" Then
                    clsStation.SellerCaption = "فروشنده"
                End If
                ...
                If CurrentDateNumber > LastDateNumber And Station_IsServer = True Then
                    ...
                    frmDisMsg.lblMessage = "سيستم در حال مرتب سازي داده ها مي باشد "
                    frmDisMsg.lblMessage = frmDisMsg.lblMessage & vbLf & "لطفا منتظر بمانيد "
                    frmDisMsg.Show
                    DoEvents
                    On Error Resume Next
                ...
                If CurrentDateNumber > LastDateNumber And Station_IsServer = True And clsStation.AutoBackup = False Then
                   ShowMessage "آيا مايل به گرفتن نسخه پشتيبان از اطلاعات خود هستيد؟", True, True, "بلي", "خير"
                   If mvarMsgIdx = vbYes Then
                        frmDisMsg.Timer1.Interval = 1000
                        frmDisMsg.lblMessage = "سيستم در حال گرفتن نسخه پشتيبان از داده ها مي باشد "
                        frmDisMsg.lblMessage = frmDisMsg.lblMessage & vbLf & "لطفا منتظر بمانيد "
                        frmDisMsg.Show
                        DoEvents
                        On Error Resume Next
    ...
    ''                Form.SetSaleSale AccountYear, CurrentBranch, mvarCurUserNo, DataBaseName, clsArya.DBLogin, clsArya.ServerName, Trim(clsArya.Company)
                    If CurrentDateNumber > LastDateNumber And Station_IsServer = True And (LCase(clsArya.ServerName) = LCase(MachineName) Or clsArya.ServerName = ".") Then
                        ShowMessage "آيا مايل به گرفتن گزارش چكهاي دريافتني تا تاريخ فردا هستيد؟", True, True, "بلي", "خير"
                        If mvarMsgIdx = vbYes Then
                            Accounting.ChequePrintingDll
                        End If
                    End If
                    If CurrentDateNumber > LastDateNumber And Station_IsServer = True And (LCase(clsArya.ServerName) = LCase(MachineName) Or clsArya.ServerName = ".") Then
                        ShowMessage "آيا مايل به گرفتن گزارش چكهاي پرداختني تا تاريخ فردا هستيد؟", True, True, "بلي", "خير"
                        If mvarMsgIdx = vbYes Then
                    ...
                    If mvarTafsili = 0 Then
                        ShowMessage " کاربر فعلي براي ثبت در سيستم حسابداري داراي کد تفصيلي نيست . يکبار کاربر مورد نظر را ويرايش کنيد يا همه کاربران را به سيستم حسابداري اضافه کنيد ", True, False, "قبول", ""
                        frmPer.Show
                        TafsiliFlag = True
                    End If
                End If
...
'        Me.LblSoftwareRegistrationNotice.ForeColor = vbRed
    ...
    If clsArya.HardLock = False Then
        If clsArya.LimitedVersion = True And HardLockFlagTrial = False Then
'            lblAddress.Caption = "       نرم افزارحاضر تحت قوانين شورايعالي انفورماتيک کشور مي باشد" & "اين نسخه از برنامه براي استفاده " & Trim(clsArya.Company) & "  تهيه گرديده و قابل کپي و تکثير  مي باشد. "
'            lblAddress.Caption = lblAddress.Caption & " استفاده از اين نسخه داراي محدوديت مي باشد و پس از مشاهده قابليت هاي ورژن هاي مختلف نرم افزاربراي تهيه نسخه اصلي با نمايندگان فروش شرکت تماس حاصل فرمائيد"
            strPolicy = "       نرم افزارحاضر تحت قوانين شورايعالي انفورماتيک کشور مي باشد" & "اين نسخه از برنامه براي استفاده " & Trim(clsArya.Company) & "  تهيه گرديده و قابل کپي و تکثير  مي باشد. "
            strPolicy = strPolicy & " استفاده از اين نسخه داراي محدوديت مي باشد و پس از مشاهده قابليت هاي ورژن هاي مختلف نرم افزاربراي تهيه نسخه اصلي با نمايندگان فروش شرکت تماس حاصل فرمائيد"
        ElseIf clsArya.LimitedVersion = True And HardLockFlagTrial = True Then
            strPolicy = "       نرم افزارحاضر تحت قوانين شورايعالي انفورماتيک کشور مي باشد" & "اين نسخه از برنامه براي استفاده در " & Trim(clsArya.Company) & " واقع در " & Trim(clsArya.CustomerAddres) & "  تهيه گرديده و قابل کپي و تکثير  مي باشد. "
        Else
            strPolicy = "       نرم افزارحاضر تحت قوانين شورايعالي انفورماتيک کشور مي باشد" & "اين نسخه از برنامه براي استفاده در " & Trim(clsArya.Company) & " واقع در " & Trim(clsArya.CustomerAddres) & "  تهيه گرديده  " & "و هرگونه کپي برداري واستفاده غير قانوني از آن  قابل پيگرد مي باشد "
        End If
    Else
        strPolicy = "       نرم افزارحاضر تحت قوانين شورايعالي انفورماتيک کشور مي باشد" & "اين نسخه از برنامه براي استفاده در " & Trim(clsArya.Company) & " واقع در " & Trim(clsArya.CustomerAddres) & "  تهيه گرديده  " & "و هرگونه کپي برداري واستفاده غير قانوني از آن  قابل پيگرد مي باشد "
    End If
    ...
    Select Case intVersion
        Case 3
            LblVersion.Caption = "ميني"
        Case 0
            LblVersion.Caption = "متوسط"
        Case 1
            LblVersion.Caption = "پيشرفته"
        Case 2
            LblVersion.Caption = "ويژه"
        Case 4
            LblVersion.Caption = "الماس"
    End Select
...
''############
''   ابتدا آريا ستينگ در درايو سي چک مي شودو اگر پيدا نشد در مسير جاري دنبال مي گردد
...
'            If clsArya.LimitedVersion = False Then MsgBox "Arya Setting File " & vbCrLf & "در مسير  " & App.Path & "\Aryasetting.txt" & "ايجاد گرديد"
'        End If
'    End If

'############ براي اينکه امکان اجراي دو برنامه هم زمان در سيستم وجود داشته باشد
'   ابتدا آريا ستينگ در مسير جاري چک مي شودو اگر پيدا نشد در درايو سي دنبال مي گردد
    AryaSettingFile = App.Path & "\Aryasetting.txt"
    IsFileExist = filetemp.FileExists(AryaSettingFile)
    ...
    If IsFileExist = False Then
        ...
        If IsFileExist = False Then
            setDefaultAryaSettingFile
            If clsArya.LimitedVersion = False Then MsgBox "Arya Setting File " & vbCrLf & "در مسير  " & "C:\Aryasetting.txt" & "ايجاد گرديد"
        End If
    End If
'############
    ...
    If clsArya.HardLockSerialNo = "93061701000" Then   ' Palladium Mall Server
        ...
        IsFileExist = filetemp.FileExists(AryaSettingFile)
        If IsFileExist = False Then
            MsgBox "Arya Setting File " & vbCrLf & "در مسير  " & App.Path & "\Aryasetting.txt" & "وجود ندارد و سيستم قادر به ادامه نيست"
            End
        Else
            MsgBox "Arya Setting File " & vbCrLf & "در مسير  " & App.Path & "\Aryasetting.txt" & "شناسايي شد"
...
'    If Trim(clsArya.Company) = "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" Then MsgBox "فايل اجرائي خام است": End
    If Trim(clsArya.HardLockSerialNo) = "xxxxxxxxxxxxxxxxxx" Then MsgBox "فايل اجرائي خام است": End
...
'            SetTooltipText
'        Else
            If Trim(clsArya.Company) = "" Then clsArya.Company = "آريا "
            If Trim(clsArya.CustomerAddres) = "" Then clsArya.CustomerAddres = "تهران"
            SetTooltipText_Demo
...
'        End If
    ...
    If Trim(clsArya.Company) = "" Then
        frmInput.fwlblInput.Caption = "لطفا نام خود را وارد نماييد "
    ...
    If Trim(clsArya.CustomerAddres) = "" Then
        frmInput.fwlblInput.Caption = "لطفا آدرس خود را وارد نماييد "
        frmInput.btnCancel.Visible = False
        frmInput.Show vbModal
