...
Begin VB.Form frmMojodiControl_1 
   ClientHeight    =   9360
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   15105
   Icon            =   "frmMojodiControl_1.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin VSFlex7LCtl.VSFlexGrid vsGood 
      ...
      FormatString    =   $"frmMojodiControl_1.frx":A4C2
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   Begin RESIZEKITLibCtl.ResizeKit ResizeKit1 
      Height          =   480
      Left            =   0
      OleObjectBlob   =   "frmMojodiControl_1.frx":A6A6
      TabIndex        =   13
      Top             =   0
      Width           =   480
   End
   ...
   End
End
Attribute VB_Name = "frmMojodiControl_1"
        ...
        While Rst.EOF = False
            'If CheckFirstMojodi.Value = 0 Or (CheckFirstMojodi.Value = 1 And Rst.Fields("FirstMojodi").Value > 0) Then
                 ...
                 Else
                    .TextMatrix(i, 5) = -Format(Rst.Fields("FirstMojodi").Value, "##.000") & "-"
                 End If
                 .TextMatrix(i, 6) = Val(Format(Rst.Fields("BuyAmount").Value, "##.000"))
...
Private Sub Form_Load()
    ...
    If ClsFormAccess.frmMojodiControl_1 = False Then
        Unload Me
        Exit Sub
    End If
...
Private Sub vsGood_DblClick()
    With vsGood
        If (.TextMatrix(.Row, IdxColRow) = "*" Or MyFormAddEditMode = EnumAddEditMode.EditMode) And (.Col = IdxColFirstStock Or .Col > IdxColGoodUnit) Then
            If .Col = IdxColFirstStock Or .Col > IdxColGoodUnit Then
               .Select .Row, .Col
               .EditCell
            End If
        Else
            If .Col = 2 Then
                'Unload frmGoodTurnOver
...
Exit Sub
Err_Handler:
    LogSaveNew "frmMojodiControl_1 => ", err.Description, err.Number, err.Source, "Printing"
    ShowErrorMessage
End Sub
