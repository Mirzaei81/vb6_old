...
INNER JOIN dbo.tGood ON T2.GoodCode = dbo.tGood.Code
	...
	SET @MablaghHavale = (SELECT SUM( fltUsedValue * FeeUnit) AS SumFee FROM tfacD --, tgood.Name
	INNER JOIN tUsePercent ON tfacD.GoodCode = dbo.tUsePercent.GoodFirstCode 
	INNER JOIN TGood ON dbo.tGood.Code = dbo.tUsePercent.GoodFirstCode
	WHERE intSerialNo = @SerialHavale AND tUsePercent.GoodCode = @GoodCode )     	        
	...
	SET @Mablagh = @Mablagh + (@GoodAmount * @MablaghHavale)
	END 
